...
public abstract class CustomCommandExecutor implements CommandExecutor{
	...
	private String createUsage(Method method) {
		...
		for (Class<?> theclass : method.getParameterTypes()){			
			if (Player.class ==theclass){
				...
				sb.append("<player> ");
			} else if (BattlePlayer.class ==theclass){
				sb.append("<player> ");
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if(DEBUG)System.out.println(command.getName() +"   " + label + "      cmdlabel = " + command.getLabel());
	...
	static final String ONLY_INGAME =RED+"You need to be in game to use this command";
	static final String ONLY_CONSOLE =RED+"You need to be on console to use this command";
	private Arguments verifyArgs(MethodWrapper mwrapper, MCCommand cmd, 
		...
		if (!cmd.perm().isEmpty() && !sender.hasPermission(cmd.perm()))
			throw new InvalidArgumentException("You don't have permission for this command");

		/// Verify min number of arguments
		if (args.length < cmd.min()){
			throw new InvalidArgumentException("You need at least "+cmd.min()+" arguments");
		}
		/// Verfiy max number of arguments
		if (args.length > cmd.max()){
			throw new InvalidArgumentException("You need less than "+cmd.max()+" arguments");
		}
		/// Verfiy max number of arguments
		if (cmd.exact()!= -1 && args.length != cmd.exact()){
			throw new InvalidArgumentException("You need exactly "+cmd.exact()+" arguments");
		}
		final boolean isPlayer = sender instanceof Player;
		final boolean isOp = (isPlayer && sender.isOp()) || sender == null || sender instanceof ConsoleCommandSender;

		if (cmd.op() && !isOp)
			throw new InvalidArgumentException(RED +"You need to be op to use this command");
		...
		if(cmd.fromConsole() && !isConsole) {
			throw new InvalidArgumentException(ONLY_CONSOLE);
		}
		...
		for (Class<?> theclass : mwrapper.method.getParameterTypes()){
			try{
				if (CommandSender.class == theclass){
					...
					objs[objIndex] = verifyBattlePlayer(args[strIndex++]);
				} else if (String.class == theclass){
				...
				throw new InvalidArgumentException("You didn't supply enough arguments for this method");
			}
			objIndex++;
		}
	...
	private BattlePlayer verifyBattlePlayer(String name) throws InvalidArgumentException {
		BattlePlayer bp;
		try {
			bp = BattlePunishments.createBattlePlayer(name, true);
		} catch (Exception e) {
			throw new InvalidArgumentException("Player "+ name + " is not registered with BattlePunishments.");
		}
	...
	private Integer verifyInteger(Object object) throws InvalidArgumentException {
		try {
			...
			throw new InvalidArgumentException(RED+(String)object+" is not a valid integer.");
		}
	}
	...
	public void help(CommandSender sender, Command command, Object[] args){
		...
		if (args != null && args.length > 1){
			try{
				...
				sendMessage(sender, RED+" " + args[1] +" is not a number, showing help for page 1.");
			}
		}
		...
		if (command != null)
			sendMessage(sender, "&ePossible commands for "+command.getName()+":");
		else 
			sendMessage(sender, "&ePossible commands:");
		int i=0;
		...
		for (String use : unavailable){
			...
			if (i < (page-1) *LINES_PER_PAGE || i >= page *LINES_PER_PAGE)
				continue;
			sendMessage(sender, RED+"[Insufficient Perms] " + use);
		}
		if (sender.isOp()){
			for (String use : onlyop){
				...
				if (i < (page-1) *LINES_PER_PAGE || i >= page *LINES_PER_PAGE)
					continue;
				sendMessage(sender, AQUA+"[OP only] &6"+use);
			}			
		}
	}
	...
	public static @interface MCCommand {
		...
		boolean fromConsole() default false;
